...
export function attachDevtoolsBridge(
  ...
  const getDeepObjectProperties = async (ws: WebSocket, objectId: string, depth: number): Promise<unknown> => {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(result.result.length, MAX_PROPERTIES); i += 1) {
            ...
            if (value.type === 'undefined') {
              ...
              if (value.subtype === 'array') {
                ...
                if (value.objectId && depth < MAX_DEPTH - 1) {
                  ...
                  properties[name] = {
                    __type: 'Map',
                    size: value.preview?.properties?.find((p: any) => p.name === 'size')?.value || 0,
                  };
                }
  ...
  const connect = () => {
    ...
    ws.on('open', () => {
      console.log(chalk.green(`[rn-inspector] Connected to DevTools websocket: ${devtoolsWsUrl}`));
      try {
        ...
        ws.send(
          JSON.stringify({
            ...
            params: { expression: INJECT_NETWORK_SNIPPET, includeCommandLineAPI: false, awaitPromise: false },
          }),
        );
        ws.send(
          JSON.stringify({
            id: 7,
            method: 'Runtime.evaluate',
            params: { expression: INJECT_STORAGE_SNIPPET, includeCommandLineAPI: false, awaitPromise: false },
          }),
        );
        ws.send(
          JSON.stringify({
            id: 8,
            method: 'Runtime.evaluate',
            params: { expression: INJECT_INSPECTOR_SNIPPET, includeCommandLineAPI: false, awaitPromise: false },
          }),
        );
        broadcast({
          type: 'meta',
