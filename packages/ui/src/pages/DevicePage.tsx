...
const mirrorPlatforms: Array<{ value: 'android' | 'ios' | 'ios-sim' | 'ios-device'; label: string }> = [
...
export default function DevicePage() {
    ...
    const [platform, setPlatform] = useState<'android' | 'ios' | 'ios-sim' | 'ios-device'>('android');
    // @ts-ignore
    const [fps, setFps] = useState<number | null>(null);
    const lastFrameTsRef = useRef<number | null>(null);
    const [platformAnchor, setPlatformAnchor] = useState<HTMLElement | null>(null);
    ...
    const handlePlatformSelect = (value: 'android' | 'ios' | 'ios-sim' | 'ios-device') => {
        setPlatform(value);
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
                {/* Device details */}
                <GlassPanel sx={{ flex: 1, minWidth: 320, display: 'flex', flexDirection: 'column', gap: 1 }}>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} alignItems="center">
                        ...
                        >
                            ...
                            >
                                {mirrorPlatforms.find((p) => p.value === platform)?.label || 'Select platform'}
                            </Typography>
                        </Button>

                        <Popper open={platformOpen} anchorEl={platformAnchor} placement="bottom-start" sx={{ zIndex: (theme) => theme.zIndex.tooltip }}>
...
- Errors from adb/xcrun are surfaced in the UI panel below.`}
                ...
                {/* Mirror panel */}
                <GlassPanel sx={{ flex: 2, minHeight: 360, display: 'flex', flexDirection: 'column', gap: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Typography variant="subtitle1" fontWeight={600}>
                            Live Screen Mirror
                        </Typography>
                        ...
                        {mirror?.error && (
                            <Chip size="small" color="error" label={mirror.error} variant="filled" />
                        )}
                    </Box>
                    <Box
                        sx={{
