...
const getTypeColor = (type: string | null): string => {
  ...
  if (lowerType.includes("button") || lowerType.includes("touchable"))
    return "#e5c07b";
  if (lowerType.includes("scroll")) return "#56b6c2";
  if (lowerType.includes("input") || lowerType.includes("textinput"))
    return "#d19a66";
    ...
    <Box sx={{ pl: depth > 0 ? 2 : 0 }}>
      ...
      >
        ...
        {node.props?.text && (
          ...
          >
            "{String(node.props.text).slice(0, 30)}
            {String(node.props.text).length > 30 ? "..." : ""}"
          </Typography>
        )}
      </Box>
      {hasChildren && (
...
export default function InspectorPage() {
  ...
  const nodeCount = useMemo(() => {
    ...
    const count = (node: UINode): number => {
      return (
        1 + (node.children?.reduce((acc, child) => acc + count(child), 0) || 0)
      );
    };
          ...
          <Box sx={{ display: "flex", gap: 1 }}>
            ...
            {currentInspector.deviceId && (
              <Chip
                size="small"
                label={
                  devices.find((d) => d.id === currentInspector.deviceId)
                    ?.label || currentInspector.deviceId
                }
                variant="filled"
              />
            )}
              ...
              {!currentInspector ? (
                ...
                >
                  <Typography color="text.secondary">
                    {status !== "open"
                      ? "Connect to a device to inspect UI"
                      : "Click Refresh to fetch component tree"}
                  </Typography>
                </Box>
                ...
                <Box sx={{ p: 2 }}>
                  ...
                  >
                    Make sure your React Native app is running with debugging
                    enabled.
                  </Typography>
                </Box>
              ) : currentInspector.hierarchy &&
                (currentInspector.hierarchy as any).note ? (
                <Box sx={{ p: 2 }}>
                  ...
                  >
                    {(currentInspector.hierarchy as any).note ||
                      "The Inspector prototype could not access the component tree from this debugger."}
                  </Typography>
                  <Typography
                    variant="caption"
                    color="text.secondary"
                    sx={{ display: "block" }}
                  >
                    This experimental UI Inspector depends on internal React
                    DevTools hooks and may not work in all React Native debugger
                    setups. There is no additional code you need to add to your
                    app; full UI inspection will require deeper integration
                    (similar to official React DevTools for React Native).
                  </Typography>
                </Box>
              ) : currentInspector.hierarchy ? (
                <UITreeNode
                    ...
                    selectedNode.children.length > 0 && (
                      <Box>
                        ...
                        <Typography
                          sx={{ fontFamily: "monospace", fontSize: 13 }}
                        >
                          {selectedNode.children.length} child component
